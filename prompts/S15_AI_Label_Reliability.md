...
## Prior-State Check
- Confirm `S14` is accepted and the repository status is clean or truthfully documented.
- Record the current checkpoint identifier.
- Confirm `S15` is the only active implementation unit in `TASKS.md`.
- Run the currently passing baseline tests before making changes.

## Session Objective
Compare AI suggestions with human gold labels using agreement, confusion matrices, and Cohen's kappa where categories support it.
...
- `notebooks/04_ai_label_validation.ipynb`
- `src/lucy_marketing_analytics/ai_validation.py`
...
5. Run the required checks and save evidence under `evidence/S15/`.
...
## Tests and Checks
- Run `python -m pytest tests/test_ai_validation.py -q`.
- Manually verify confusion-matrix order, denominators, and ambiguous cases.
- Review `git diff --check`, `git status --short`, and the intended diff.

## Acceptance Criteria
- Agreement results identify labels, denominators, and ambiguous cases.
- Kappa is omitted or qualified when prevalence or category counts make it misleading.
- Required evidence exists under `evidence/S15/`, and failed checks are not hidden.
...
## Do Not Do
- Do not begin `S16` or implement future-unit features.
...
- If checks pass, stage only intended paths, commit as `S15: ai label reliability`, and optionally tag `checkpoint-s15`.
- If a check fails, keep `S15` active, record the failure and last verified checkpoint, and repair only this unit.
...
## Stop Condition
Stop when `S15` acceptance passes. Do not begin `S16`.

# S01 - Research Scope and Evidence Workflow
...
## Prior-State Check
- Confirm `INIT and workspace audit` is accepted and the repository status is clean or truthfully documented.
- Record the current checkpoint identifier.
- Confirm `S01` is the only active implementation unit in `TASKS.md`.
- Run the currently passing baseline tests before making changes.

## Session Objective
Freeze the research questions, sampling and claim boundaries, success criteria, repository workflow, evidence paths, review cadence, and public/private separation.
...
5. Run the required checks and save evidence under `evidence/S01/`.
...
## Tests and Checks
- Search the plan for all five research questions and verify each remains traceable.
- Verify all 18 units and evidence destinations remain in `TASKS.md`; review the claim vocabulary and repository cadence.
- Review `git diff --check`, `git status --short`, and the intended diff.

## Acceptance Criteria
- All five research questions and the observation-to-claim ladder are traceable.
- Every later unit has an evidence destination and review owner; the 30-item Gold Dataset is required while expansion and dashboard remain optional.
- Required evidence exists under `evidence/S01/`, and failed checks are not hidden.
...
## Do Not Do
- Do not begin `S02` or implement future-unit features.
...
- If checks pass, stage only intended paths, commit as `S01: research scope and evidence workflow`, and optionally tag `checkpoint-s01`.
- If a check fails, keep `S01` active, record the failure and last verified checkpoint, and repair only this unit.
...
## Stop Condition
Stop when `S01` acceptance passes. Do not begin `S02`.

# S03 - Label Guide and Pilot Validation
...
## Prior-State Check
- Confirm `S02` is accepted and the repository status is clean or truthfully documented.
- Record the current checkpoint identifier.
- Confirm `S03` is the only active implementation unit in `TASKS.md`.
- Run the currently passing baseline tests before making changes.

## Session Objective
Operationalize the marketing taxonomy, implement deterministic schema checks, and audit the five-record pilot for IDs, types, ranges, missingness, duplicate sources, and boundary labels.
...
- `docs/MarketingLabelGuide.md`
- `config/label_taxonomy.json`
- `src/lucy_marketing_analytics/validation.py`
...
5. Run the required checks and save evidence under `evidence/S03/`.
6. Update `TASKS.md`, `docs/decisions.md`, and `docs/CoBuildLog.md` truthfully.

## Manual Work
Lucy must resolve ambiguous labels and approve field definitions after examining real examples.

## Tests and Checks
- Parse `config/label_taxonomy.json`.
- Run `python -m pytest tests/test_validation.py -q`, save the pilot audit, and double-code two boundary examples manually.
- Review `git diff --check`, `git status --short`, and the intended diff.

## Acceptance Criteria
- Each label has inclusion/exclusion rules and boundary examples; fact, interpretation, and AI suggestion remain separate.
- Invalid IDs, score ranges, duplicates, and categories are rejected, while missingness is reported honestly.
- Required evidence exists under `evidence/S03/`, and failed checks are not hidden.
...
## Do Not Do
- Do not begin `S04` or implement future-unit features.
...
- If checks pass, stage only intended paths, commit as `S03: label guide and pilot validation`, and optionally tag `checkpoint-s03`.
- If a check fails, keep `S03` active, record the failure and last verified checkpoint, and repair only this unit.
...
## Stop Condition
Stop when `S03` acceptance passes. Do not begin `S04`.

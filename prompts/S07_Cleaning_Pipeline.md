...
## Prior-State Check
- Confirm `S06` is accepted and the repository status is clean or truthfully documented.
- Record the current checkpoint identifier.
- Confirm `S07` is the only active implementation unit in `TASKS.md`.
- Run the currently passing baseline tests before making changes.

## Session Objective
Implement reproducible cleaning for IDs, dates, numeric metrics, text status, missing values, duplicates, and audit flags.
...
- `src/lucy_marketing_analytics/data_cleaning.py`
...
5. Run the required checks and save evidence under `evidence/S07/`.
...
## Tests and Checks
- Run `python -m pytest tests/test_data_cleaning.py -q`.
- Run cleaning twice and compare outputs.
- Review `git diff --check`, `git status --short`, and the intended diff.

## Acceptance Criteria
- Cleaning is deterministic and idempotent.
- Raw/private inputs are never overwritten; clean outputs retain provenance.
- Required evidence exists under `evidence/S07/`, and failed checks are not hidden.
...
## Do Not Do
- Do not begin `S08` or implement future-unit features.
...
- If checks pass, stage only intended paths, commit as `S07: cleaning pipeline`, and optionally tag `checkpoint-s07`.
- If a check fails, keep `S07` active, record the failure and last verified checkpoint, and repair only this unit.
...
## Stop Condition
Stop when `S07` acceptance passes. Do not begin `S08`.
